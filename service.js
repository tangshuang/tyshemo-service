      ...
      const data = this.data.map((group) => {
        ...
        return {
          ...
          items: group.items.map((item) => {
            ...
            const url = (basePath ? basePath : '') + path
            const makeRequest = (req, reqDesc, reqPatchPath) => {
              const clonedDesc = clone(reqDesc)
            ...
            const units = []
            test.forEach((item, i) => {
              const reqDesc = reqType ? this.mocker.mock(reqType) : null
              const reqPatchPath = getRequestType ? getPath(getRequestType(RANDOM), RANDOM) : ''
              const { params = {}, request = {}, response, frequency, name } = item
              ...
              const unit = {
                ...
                method,
                path,
                request: makeRequest(request, reqDesc, reqPatchPath),
                response: response ? makeResponse(response) : resDesc,
              }
              unit.hash = getObjectHash({ group: group.name, item: item.name, unit: unit.name })
              units.push(unit)
            })
